...
import * as z from "zod";
// ملاحظة: sendContactMessageAction يتم استيرادها من actions.ts الرئيسي
import { sendContactMessageAction, suggestFaqAction } from "@/app/actions";
import { revalidatePath } from "next/cache";

// ========= START: UPDATE ZOD SCHEMA =========
const contactSchema = z.object({
  name: z.string().min(2, "Name must be at least 2 characters"),
  email: z.string().email("Please enter a valid email address"),
...
export async function sendMessageAction(data: FormData) {
  const parsed = contactSchema.safeParse({
    ...
    email: data.get("email"),
    message: data.get("message"),
    inquiry: data.get("projectIdea") || "لا يوجد اي استفسار حالياً",
    beneficiaryType: data.get("beneficiaryType"),
    requestType: data.get("requestType"),
    // ========= START: EXTRACT NEW FIELDS =========
    preferredContactMethod: data.get("preferredContactMethod"),
    contactMethodValue: data.get("contactMethodValue"),
    // ========= END: EXTRACT NEW FIELDS =========
  });

  if (!parsed.success) {
    console.error("Validation failed:", parsed.error.flatten());
    // توفير رسالة خطأ أكثر تفصيلاً للمطور
    const firstError = Object.values(parsed.error.flatten().fieldErrors)[0]?.[0];
    return { success: false, message: firstError || "Validation failed." };
  }

  try {
    // تمرير البيانات المحدثة (parsed.data) إلى الدالة الرئيسية
    await sendContactMessageAction(parsed.data);
    revalidatePath("/ar"); // يمكنك إزالة هذا إذا كان غير ضروري
    revalidatePath("/en"); // إضافة اللغة الإنجليزية أيضًا
    return { success: true, message: "Message sent successfully!" };
  } catch (error) {
    console.error("Failed to add message:", error);
    return { success: false, message: "Failed to send message. Please try again later." };
